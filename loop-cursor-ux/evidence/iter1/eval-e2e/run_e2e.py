"""Evaluator throwaway cursor-native e2e (port >= 18400)."""
...
OUT = ROOT / "loop-cursor-ux/evidence/iter1/eval-e2e"
...
def main() -> int:
    ...
    scratch = Path(tempfile.mkdtemp(prefix="cursor-ux-eval-e2e-"))
        ...
        with httpx.Client(base_url=base_url, timeout=30) as client:
            ...
            for ev in events:
                ...
                if et in {"error", "response.error"}:
                    open_error_codes.append(err.get("code") or et)
            ...
            assistant_done_s: float | None = None
            deltas_after_15 = 0
            deltas: list[str] = []
            complete_text = ""
            seen = 0
            while time.monotonic() - t0 < 240:
                while seen < len(events):
                    ...
                    if et == "response.completed" and completed_s is None:
                        completed_s = elapsed
                    if et == "session.input.consumed" and consumed_s is None:
                        consumed_s = elapsed
                    ...
                    if et == "response.output_item.done":
                        ...
                        if item.get("role") == "assistant":
                            ...
                            if assistant_done_s is None:
                                assistant_done_s = elapsed
                if assistant_done_s is not None and (time.monotonic() - t0) > (
                    assistant_done_s + 3.0
                ):
                    break
                time.sleep(0.05)
            ...
            stop.set()
            joined = "".join(deltas)
            last = deltas[-1] if deltas else ""
            result.update(
                {
                    "ttfd_s": first_delta_s,
                    "injection_completed_s": completed_s,
                    "consumed_s": consumed_s,
                    "assistant_item_done_s": assistant_done_s,
                    ...
                    "last_delta_len": len(last),
                    "last_delta_in_complete": bool(last) and last in complete_text,
                    "consumed_before_assistant_item": (
                        consumed_s is not None
                        and assistant_done_s is not None
                        and consumed_s <= assistant_done_s
                    ),
