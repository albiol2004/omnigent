...
EV = ROOT / "loop-fork-async/evidence/iter1/eval-e2e"
...
SCRATCH = Path("/tmp/fork-async-eval-iter1")
...
def _sqlite_early_response_id(
    db: Path, session_hex: str, last_rid: str
) -> str | None:
    """First response_id that does not cover the full item list."""
    cid = bytes.fromhex(session_hex)
    conn = sqlite3.connect(str(db))
    try:
        rows = conn.execute(
            "SELECT response_id FROM conversation_items "
            "WHERE conversation_id=? AND response_id IS NOT NULL "
            "AND response_id != '' ORDER BY position ASC",
            (cid,),
        ).fetchall()
        for (rid,) in rows:
            if rid != last_rid:
...
def main() -> None:
    ...
    jsonl_dest = proj / f"{SOURCE_EXT}.jsonl"
    shutil.copy2(LIVE_JSONL, jsonl_dest)
    jsonl_dest.chmod(0o444)
    ...
    try:
        ...
        if fork_a.status_code == 201 and a_id:
            ...
            cloned = _clone_claude_transcript(
                ...
                clone_workspace=Path("/tmp/fork-async-eval-iter1/ws"),
            )
        ...
        early_rid = _sqlite_early_response_id(db, SOURCE_HEX, last_rid)
        result["early_response_id"] = early_rid
        if not early_rid:
            raise SystemExit("source has no early response_id for a3")
        t_a3 = time.monotonic()
        fork_a3 = httpx.post(
            f"{base}/v1/sessions/{SOURCE_HEX}/fork",
            json={
                "title": "e2e-truncated-prefix",
                "up_to_response_id": early_rid,
            },
            timeout=10.0,
        )
        result["a3_wall_s"] = round(time.monotonic() - t_a3, 3)
        result["a3_status"] = fork_a3.status_code
        a3_json = fork_a3.json() if fork_a3.status_code == 201 else {}
        result["a3_fork_id"] = a3_json.get("id")
        result["a3_labels"] = a3_json.get("labels") or {}
        result["a3_preparing"] = (a3_json.get("labels") or {}).get(PREPARING)
        _record_fork(EV / "a3_fork.txt", fork_a3)
        log_after_a3 = (SCRATCH / "server.log").read_text(errors="replace")
        result["a3_skip_log"] = [
            line for line in log_after_a3.splitlines()
            if "fork passthrough skipped" in line
        ][-5:]
        if fork_a3.status_code == 201 and result["a3_fork_id"]:
            # Drain this compaction before case (b) so the CLI is not shared.
            a3_wait, a3_done = _wait_preparing(
                base, result["a3_fork_id"], timeout=240.0
            )
            result["a3_bg_s"] = a3_wait
            result["a3_labels_done"] = a3_done.get("labels") or {}
            result["a3_item_count"] = _sqlite_item_stats(
                db, result["a3_fork_id"]
            )["count"]
